...
from dolfinx.io import (VTKFile, distribute_entity_data, gmsh as gmshio)
...
sample_gap = 10
mono_save_file = f"NSE_snaps_gap_{sample_gap}_quads_T_{T}.bp"
KE_savefile = f"NSE_KE_gap_{sample_gap}_quad_T_{T}.npz"
...
if plot == True:
    target_u = folder / ("r_" + str(r) + "_u.pvd")
    target_p = folder / ("r_" + str(r) + "_p.pvd")

    for stale in folder.glob(target_u.stem + "*"):
        stale.unlink()
    for stale in folder.glob(target_p.stem + "*"):
        stale.unlink()

    vtx_u = VTKFile(mesh.comm, str(target_u), "w")
    vtx_p = VTKFile(mesh.comm, str(target_p), "w")
...
if mesh_comm.rank == 0:
    if save_KE: 
        KE_path = result_folder / KE_savefile
        np.savez(KE_path, times = KE_times, KE = KE_arr)
    if save_monolithic:
